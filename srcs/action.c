...
/*   Updated: 2023/06/26 19:19:27 by minabe           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	take_forks(t_philo *philo_data)
{
	...
	printf("%d has taken a fork\n", philo_data->id + 1);
	pthread_mutex_unlock(&philo_data->data.log);
	// print_log(philo_data, "has taken a fork", philo_data->data.forks[first(philo_data)]);
	// print_log(philo_data, "has taken a fork", philo_data->data.forks[second(philo_data)]);
}
...
void	eating(t_philo *philo_data)
{
	// print_time_diff(philo_data->data.start_time);
	// printf("%d is eating\n", philo_data->id + 1);
	// print_log(philo_data, "is eating", philo_data->data.log);
	print_log(philo_data, "is eating", philo_data->data.log);
